...
if (isset($_POST['btnAdd'])) {
  ...
  $course_id = '';
    ...
    $sql_get_course_id = "SELECT course_id FROM course WHERE course_name = '$course_name' LIMIT 1";
    $result_get_course_id = mysqli_query($con, $sql_get_course_id);
        if ($result_get_course_id && mysqli_num_rows($result_get_course_id) > 0) {
            $row_course = mysqli_fetch_assoc($result_get_course_id);
            $course_id = $row_course['course_id'];  // Lấy course_id tương ứng
        }
    }

  $sql_check = "SELECT * FROM `training`";
  $result_check = mysqli_query($con, $sql_check);

      $sql_insert = "INSERT INTO `training`(`staff_id`, `course_id`) VALUES ('$staff_id', '$course_id')";
      $data = mysqli_query($con, $sql_insert);
      if ($data) {
          echo "<script>alert('Thêm thông tin đào tạo thành công!'); window.location='Training.php';</script>";
      } else {
          echo "<script>alert('Thêm thông tin đào tạo thất bại!')</script>";
      }
}
if (isset($_POST['btnBack'])) {
  header('location: ../HRM/Staff.php');
    ...
    $data_course = mysqli_query($con, $sql_course);
                    ...
                    <h5 class="card-title fw-semibold">Thông tin nhân viên</h5>
                      ...
                      <div class="row">                        
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                              <div class="mb-3">
                                <label class="form-label">Khoá đào tạo</label>
                                <select name="course_name" class="form-select" data-placeholder="Choose a Category" tabindex="1">
                                <option value="">--Chọn khoá đào tạo--</option>
                                <?php 
                                    if(isset($data_course)&&mysqli_num_rows($data_course)>0){
                                        while($row=mysqli_fetch_assoc($data_course)){
                                ?>
                                            <option value="<?php echo $row['course_name'] ?>" <?php if($course_name==$row['course_name']) echo 'selected' ?>>
